...
namespace RxCanvas.Editors
{
    public class PortableXLineEditor : IEditor, IDisposable
    {
        ...
        public PortableXLineEditor(INativeFactory nativeFactory, IPortableFactory portableFactory, ICanvas canvas)
        {
            _canvas = canvas;
            ...
            _downs = _canvas.Downs.Subscribe(p =>
            {
                ...
                {
                    _xline = portableFactory.CreateLine();
                    _xline.X1 = p.X;
                    _xline.Y1 = p.Y;
                    _xline.X2 = p.X;
                    _xline.Y2 = p.Y;
                    _line = nativeFactory.CreateLine(_xline);
                    _canvas.Add(_line);
                    _canvas.Capture();
                    _state = State.End;
                }
    ...
    public class PortableXBezierEditor : IEditor, IDisposable
    {
        ...
        public PortableXBezierEditor(INativeFactory nativeFactory, IPortableFactory portableFactory, ICanvas canvas)
        {
            _canvas = canvas;
            ...
            _downs = _canvas.Downs.Subscribe(p =>
            {
                ...
                {
                    _xb = portableFactory.CreateBezier();
                    _xb.Start.X = p.X;
                    _xb.Start.Y = p.Y;
                    _xb.Point1.X = p.X;
                    _xb.Point1.Y = p.Y;
                    ...
                    _xb.Point3.X = p.X;
                    _xb.Point3.Y = p.Y;
                    _b = nativeFactory.CreateBezier(_xb);
                    _canvas.Add(_b);
                    _canvas.Capture();
                    _state = State.Start;
                }
    ...
    public class PortableXQuadraticBezierEditor : IEditor, IDisposable
    {
        ...
        public PortableXQuadraticBezierEditor(INativeFactory nativeFactory, IPortableFactory portableFactory, ICanvas canvas)
        {
            _canvas = canvas;
            ...
            _downs = _canvas.Downs.Subscribe(p =>
            {
                ...
                {
                    _xqb = portableFactory.CreateQuadraticBezier();
                    _xqb.Start.X = p.X;
                    _xqb.Start.Y = p.Y;
                    _xqb.Point1.X = p.X;
                    _xqb.Point1.Y = p.Y;
                    _xqb.Point2.X = p.X;
                    _xqb.Point2.Y = p.Y;
                    _qb = nativeFactory.CreateQuadraticBezier(_xqb);
                    _canvas.Add(_qb);
                    _canvas.Capture();
                    _state = State.Start;
                }
    ...
    public class PortableXArcEditor : IEditor, IDisposable
    {
        ...
        public PortableXArcEditor(INativeFactory nativeFactory, IPortableFactory portableFactory, ICanvas canvas)
        {
            _canvas = canvas;
            ...
            _downs = _canvas.Downs.Subscribe(p =>
            {
                ...
                else
                {
                    ...
                    _xarc = portableFactory.CreateArc();
                    _xarc.X = _start.X;
                    _xarc.Y = _start.Y;
                    _arc = nativeFactory.CreateArc(_xarc);
                    _canvas.Add(_arc);
                    _canvas.Capture();
                    _state = State.Size;
                }
    ...
    public class PortableXCanvasRectangleEditor : IEditor, IDisposable
    {
        ...
        public PortableXCanvasRectangleEditor(INativeFactory nativeFactory, IPortableFactory portableFactory, ICanvas canvas)
        {
            _canvas = canvas;
            ...
            _downs = _canvas.Downs.Subscribe(p =>
            {
                ...
                else
                {
                    ...
                    _xrectangle = portableFactory.CreateRectangle();
                    _xrectangle.X = _start.X;
                    _xrectangle.Y = _start.Y;
                    _rectangle = nativeFactory.CreateRectangle(_xrectangle);
    ...
    public class PortableXCanvasEllipseEditor : IEditor, IDisposable
    {
        ...
        public PortableXCanvasEllipseEditor(INativeFactory nativeFactory, IPortableFactory portableFactory, ICanvas canvas)
        {
            _canvas = canvas;
            ...
            _downs = _canvas.Downs.Subscribe(p =>
            {
                ...
                {
                    _start = new ImmutablePoint(p.X - 1.0, p.Y - 1.0);
                    _xellipse = portableFactory.CreateEllipse();
                    _xellipse.X = _start.X;
                    _xellipse.Y = _start.Y;
                    _elllipse = nativeFactory.CreateEllipse(_xellipse);
